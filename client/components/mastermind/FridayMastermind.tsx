...
export const FridayMastermind: React.FC = () => {
  ...
  const handleCopyNotice = () => {
    const portalUrl = typeof window !== "undefined"
      ? new URL("/member/login", window.location.origin).toString()
      : "/member/login";
    navigator.clipboard.writeText(`CapSol Friday Mastermind Schedule:\n• Friday, September 4th at 6:00 PM EST\n• Friday, September 11th at 4:00 PM EST\nZoom credentials are sent to active members via email and available inside the portal: ${portalUrl}`);
    setCopied(true);
  ...
  const getGoogleCalendarUrl = (sessionDate: 'sept4' | 'sept11') => {
    ...
    const portalUrl = typeof window !== "undefined"
      ? new URL("/member/login", window.location.origin).toString()
      : "/member/login";
    const details = encodeURIComponent(`Live weekly mastermind session for CapSol members. We break down live credit reports, audit dispute progress, and map out bank funding routes. Portal: ${portalUrl}`);
