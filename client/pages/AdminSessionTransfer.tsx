...
export default function AdminSessionTransfer() {
  ...
  const hydrateDirectHashAuthFallback = () => {
    ...
    postTransferDebug("admin_direct_hash_applied", {
      tokenApplied: true,
      role,
      userId,
  ...
  useEffect(() => {
    ...
    postTransferDebug("admin_force_consume_before", {
    ...
    const finalizeAdminSession = async () => {
      await flushTransferDebugQueue();

      const token = window.localStorage.getItem("auth_token");
      postTransferDebug("admin_session_start", {
      ...
      if (!token) {
        postTransferDebug("admin_session_no_token", {
          redirectTo: "/login",
        });
        navigateToAdminPath("/login");
        return;
      }

      try {
        postTransferDebug("admin_session_verify_call", {});
        const response = await authApi.verifyToken();
        const user = response.data?.user;
        const isAllowed = response.data?.valid && user && ALLOWED_ROLES.includes(user.role);

        postTransferDebug("admin_session_verify_result", {
        ...
        if (!isAllowed) {
          postTransferDebug("admin_session_not_allowed", {
            redirectTo: "/login",
          });
          clearStoredAuth();
          if (!cancelled) {
