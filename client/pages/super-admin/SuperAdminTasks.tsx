...
import { FileText as FileTextIcon, Image as ImageIcon, Loader2, Pencil, Plus, Trash2 } from "lucide-react";
...
export default function SuperAdminTasks() {
  ...
  const handleStatusChange = async (task: Task, status: TaskStatus) => {
    ...
    try {
      ...
      const response = await superAdminApi.updateTask(task.id, {
        status,
  ...
  const handlePreviewStatusSave = async () => {
    ...
    try {
      const response = await superAdminApi.updateTask(previewTask.id, {
