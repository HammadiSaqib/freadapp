// Script to fix credit report display issues
import fs from 'fs';
...
function displayCreditReport(report) {
  ...
  output += ' Equifax\n';
  output += ` ${report.scores.equifax}\n\n`;
  output += ' TransUnion\n';
  output += ` ${report.scores.transunion}\n\n`;
  output += ' Experian\n';
  output += ` ${report.scores.experian}\n\n`;
  output += ' Account Summary\n';
  output += ' Bureau \t Total Accounts \t Open Accounts \t Closed Accounts \t Balances\n';
  output += ` transunion \t ${report.accountSummary.transunion.totalAccounts} \t ${report.accountSummary.transunion.openAccounts} \t ${report.accountSummary.transunion.closedAccounts} \t ${report.accountSummary.transunion.balances}\n`;
  output += ` experian \t ${report.accountSummary.experian.totalAccounts} \t ${report.accountSummary.experian.openAccounts} \t ${report.accountSummary.experian.closedAccounts} \t ${report.accountSummary.experian.balances}\n`;
  output += ` equifax \t ${report.accountSummary.equifax.totalAccounts} \t ${report.accountSummary.equifax.openAccounts} \t ${report.accountSummary.equifax.closedAccounts} \t ${report.accountSummary.equifax.balances}`;
  
  return output;
}

// Function to fix the credit report display in the client
function fixCreditReportDisplay(reportData) {
  // Parse the report data
  const parsedReport = parseCreditReport(reportData);
  
  // Format the report for display
  return displayCreditReport(parsedReport);
}
...
async function main() {
  try {
    ...
    const formattedReport = fixCreditReportDisplay(JSON.parse(reportData));
    
    console.log(formattedReport);
    
    // Write the formatted report to a file
    await fs.promises.writeFile('formatted_report.txt', formattedReport);
